...
| 3 | **`refreshSession()` post-success.** Spec said `getUser()` after `ok: true`. Shipped: `refreshSession()` after success **plus** a stale-JWT recovery branch in bootstrap that probes `getUser()` and refreshes when the live user has `is_beta_user` but the persisted JWT does not. | [news-app/lib/auth.ts](../../../news-app/lib/auth.ts) `bootstrap()` | `getUser()` returns the live user record but does NOT refresh the session JWT in localStorage. On reload, `getSession()` returns the pre-redemption JWT and the gate re-triggers. Only `refreshSession()` mints a new JWT carrying the new `app_metadata` claims. The recovery branch unbricks users who redeemed under the original (broken) code. |
...
- A `security definer` function called from RLS gives every future user-scoped table a one-line policy: `using (is_beta_user() and user_id = auth.uid())`.
