...
test('auth uses GitHub and Google OAuth only', () => {
  ...
  assert.match(config, /flowType:\s*'pkce'/)
  assert.match(auth, /redirectTo \? \{ redirectTo \} : undefined/)
  assert.match(auth, /window\.location\.origin/)
  assert.doesNotMatch(auth, /APP_URL/)
  assert.doesNotMatch(auth, /exchangeCodeForSession/)
  assert.doesNotMatch(auth, /url\.searchParams\.delete\('code'\)/)
